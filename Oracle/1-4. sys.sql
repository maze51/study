...
/*
n분 전 시점으로 돌아가기
(커밋 이전 시점/드랍 이후에도 이전으로 되돌리기)
flashback table MEMBER to timestamp sysdate 1/24/60*10;

alter table MEMBER enable row movement;
----------------------------------------------------------------------
25일
오늘의 배울 것: SELECT절 논리, 비교연산 + 함수

SQL 디벨로퍼 주석 색상 바꾸기
도구-환경설정-코드편집기-pl/sql구문 색상-스타일에서 기본 주석 선택-적당히 바꾸기

DESC 테이블명;	해당 테이블의 데이터형 조회
...
QueryBox
설치 후 dbms연결
별명지정-호스트에 localhost or 127.0.0.1
SID(Sequence ID) xe
사용자 이름/비번 PC10/JAVA 입력
CTRL E - 실행 계획 확인

JOIN할 테이블 순서를 바꿔도 속도엔 차이가 없다.
INDEX FULL SCAN - 처음부터 끝까지 쭉 훑어 찾기

대상을 한정하면?(LPROD_GU = 'P302')
COST(처리비용)가 크게 준다. CARD = 카디널리티.
INDEX UNIQUE SCAN 기본키 가지고 검색한 것.

쿼리박스를 쓰는 이유? 실행계획 확인에 좋음
확인 후 어떤 JOIN을 쓸 때 COST가 떨어지는가? 이것이 최적화 과정


TABLE ALIAS는 긴 테이블명을 간략하게 표기하고 서로 구분하기 위해 필요

LIKE % _ - 기억하기!!

CTRL 누르고 개체를 클릭하면 정보가 뜸
-------------------------------------
A -> B
B는 A에 종속되어 있다.
-------------------------------------
...
DB에 아주 큰 단위의 데이터를 넣을 때 CLOB 사용


오라클에서 boolean나타내기(0과 1만)
col CHAR(1)
